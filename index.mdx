---
title: "Guia do Usuário"
description: ""
mode: "custom"
---

export const HeroCard = ({imageSrc, title, description, href}) => {
  return <a className="group cursor-pointer pb-8" href={href}>
      <img src={imageSrc} className="pointer-events-none group-hover:scale-105 transition-all duration-100" />
      <h3 className="mt-5 text-gray-900 dark:text-zinc-50 font-medium">{}</h3>
      <span className="mt-1.5">
        {description}
      </span>
    </a>;
};


<div className="relative">
  <div className="absolute top-0 lg:-top-16 left-0 right-0">
    <img src="https://mintlify.s3.us-west-1.amazonaws.com/mintlify/images/hero/background-light.png" className="block dark:hidden pointer-events-none" />
    <img src="https://mintlify.s3.us-west-1.amazonaws.com/mintlify/images/hero/background-dark.png" className="hidden dark:block pointer-events-none" />
  </div>

  <div className="px-4 py-16 lg:py-48 lg:pb-24 max-w-3xl mx-auto">
    <div className="text-4xl font-medium text-center text-gray-900 dark:text-zinc-50 tracking-tight">
      Guia do Usuário
    </div>

    <p className="max-w-xl mx-auto px-4 mt-4 text-lg text-center text-gray-500 dark:text-zinc-500">
      Este guia foi criado para ajudar você a utilizar todos os recursos do sistema de forma simples, segura e eficiente.
    </p>

    <div className="px-6 lg:px-0 mt-12 lg:mt-24 grid sm:grid-cols-2 gap-x-6 gap-y-4">
      <HeroCard 
        imageSrc="https://i.imgur.com/6h7Rswx.png" 
        title="Sales" 
        description="Gerencie suas vendas e acompanhe o desempenho" 
        href="/usuarios" 
      />

      <HeroCard 
        imageSrc="https://i.imgur.com/8qI6PaT.png" 
        title="Field ( Planejamento )" 
        description="Ferramentas para trabalho em campo" 
        href="/campo" 
      />
    </div>
  </div>
</div>